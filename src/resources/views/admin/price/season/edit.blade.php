<x-admin-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form action="{{ route('admin.price.season.update', $season) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="space-y-6">
                            {{-- 基本情報 --}}
                                ...
                                <div class="grid grid-cols-1 gap-6">
                                    ...
                                    <div>
                                        ...
                                        <textarea name="description" id="description" rows="3"
                                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                                            required>{{ old('description', $season->description) }}</textarea>
                                    </div>
                                    <div>
                                        <label for="rate" class="block text-sm font-medium text-gray-700">料金倍率（%）</label>
                                        <input type="number" name="rate" id="rate" value="{{ old('rate', $season->rate) }}"
                                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                            ...
                            <div>
                                ...
                                <div id="periods-container" class="space-y-4">
                                    @foreach($season->periods as $index => $period)
                                    <div class="period-row grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <input type="date" name="periods[{{ $index }}][start_date]"
                                                value="{{ old('periods.'.$index.'.start_date', $period->start_date->format('Y-m-d')) }}"
                                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                                                required>
                                        </div>
                                        <div class="relative">
                                            <label class="block text-sm font-medium text-gray-700">終了日</label>
                                            <input type="date" name="periods[{{ $index }}][end_date]"
                                                value="{{ old('periods.'.$index.'.end_date', $period->end_date->format('Y-m-d')) }}"
                                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                                                required>
                                            @if(!$loop->first)
    ...
    @push('scripts')
    <script>
        let periodCount = {{ count($season->periods) }};
