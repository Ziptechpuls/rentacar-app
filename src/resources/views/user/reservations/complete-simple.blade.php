        ...
        <div class="bg-white shadow-lg rounded-2xl p-6 space-y-6">
            ...
            <div class="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-xl p-6 space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <p><span class="font-medium text-gray-700">予約ID:</span> <span class="text-blue-600 font-bold">#{{ $reservationId ?? '12345' }}</span></p>
                    <p><span class="font-medium text-gray-700">車両名:</span> {{ $carName ?? 'プリウス2' }}</p>
                    <p><span class="font-medium text-gray-700">利用開始:</span> {{ $startDateTime ?? '2025年7月26日 19:12' }}</p>
                    <p><span class="font-medium text-gray-700">利用終了:</span> {{ $endDateTime ?? '2025年7月27日 19:12' }}</p>
                </div>
                <div class="mt-4 p-4 bg-white rounded-lg border">
                    <p class="text-center"><span class="font-medium text-gray-700">合計金額:</span> 
                        <span class="text-2xl font-bold text-green-600">¥{{ number_format($totalPrice ?? 22800) }}</span>
                    </p>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-xl p-6 space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <p><span class="font-medium text-gray-700">お名前:</span> {{ $customerName ?? '荻堂 豪' }}</p>
                    <p><span class="font-medium text-gray-700">カナ:</span> {{ $customerKana ?? 'オギドウ タケシ' }}</p>
                    <p><span class="font-medium text-gray-700">メールアドレス:</span> {{ $email ?? 'goiu@mail.com' }}</p>
                    <p><span class="font-medium text-gray-700">電話番号:</span> {{ $phone ?? '090-9989-8898' }}</p>
                </div>
            </div>
